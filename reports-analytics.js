...
document.addEventListener('DOMContentLoaded', function() {
  loadReports().then(() => {
    setupEventListeners(); // attach filters after data is loaded
    loadBudgets();
    loadAnalytics();
    checkForUpdates();
  });
...
// Load Reports from API and Merge Vendor Submissions
...
async function loadReports() {
  try {
    ...
    const [reportsResponse, vendorResponse] = await Promise.all([
      ...
      fetch('http://localhost:3000/api/vendor-submissions', {
        headers: {
          'Authorization': `Bearer ${getAuthToken()}`,
          'Content-Type': 'application/json'
        }
    ...
    currentReports = (reportsData.projects || []).concat(
      (vendorData.submissions || []).map((r, i) => ({
        id: `VEND-${i + 1}`,
        title: r.title,
        department: mapDepartmentName(r.type || r.department || 'Unknown'),
        vendor: r.vendor_name || r.vendorName || 'Unknown Vendor',
        status: WORKFLOW_STATUS.VENDOR_SUBMITTED,
        submitDate: formatDate(r.submit_date),
        lastUpdate: formatDate(r.submit_date),
        projectName: r.title,
        completion: 0,
        budget: `₹${r.cost}`,
    ...
    currentReports = generateDemoReports(); // fallback
...
function mapDepartmentName(type) {
  const map = {
...
function generateDemoReports() {
    ...
    const vendors = ['Vishal Nair', 'XYZ Contractors', 'DEF Infrastructure', 'GHI Builders', 'JKL Engineering'];
...
function generateWorkflowActions(report) {
    ...
    switch(report.status) {
        ...
        case WORKFLOW_STATUS.DM_REVIEWING:
            actions += `
                <button class="action-btn btn-update" onclick="updateReport('${report.id}')">
                    <i class="fas fa-edit"></i>
                    Update Report
                </button>
                <button class="action-btn btn-verify" onclick="verifyReport('${report.id}')">
        ...
        case WORKFLOW_STATUS.DM_VERIFIED:
            actions += `
                <button class="action-btn btn-forward" onclick="forwardToCM('${report.id}')">
                    <i class="fas fa-paper-plane"></i>
                    Forward to CM
                </button>
                <button class="action-btn btn-budget" onclick="requestBudget('${report.id}')">
...
async function authorizeWork(reportId) {
    try {
        ...
        showToast('Work authorized successfully. Vendor can proceed.', 'success');
        renderReports();
...
async function requestBudget(reportId) {
    ...
    try {
        ...
        showToast('Budget request submitted to CM', 'success');
...
function getStatusText(status) {
    const statusMap = {
        ...
        [WORKFLOW_STATUS.CM_PENDING]: 'CM Pending',
        [WORKFLOW_STATUS.CM_VERIFIED]: 'CM Verified',
...
function populateVendorFilter() {
  ...
  const vendorSet = new Set(currentReports.map(r => r.vendor).filter(Boolean));
...
// New function to render vendor reports
...
function formatDate(dateString) {
  ...
  return `${year}-${month}-${day}`; // Returns correct local date
}

function populateDepartmentFilter() {
  const deptFilter = document.getElementById('departmentFilterReports');
  const departments = [...new Set(currentReports.map(r => r.department).filter(Boolean))];
